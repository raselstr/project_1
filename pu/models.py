...
model_subkegiatan = 'dausg.DausgpuSub'
...
class BaseRencanapu(models.Model):
...
class Rencanapu(BaseRencanapu):
    class Meta(BaseRencanapu.Meta):
        constraints = [
            UniqueConstraint(fields=['rencana_tahun', 'rencana_subopd', 'rencana_kegiatan'], name='unique_rencana_pu')
        ]
        db_table = 'pu_rencanapu'

class Rencanapusisa(BaseRencanapu):
    class Meta(BaseRencanapu.Meta):
        constraints = [
            UniqueConstraint(fields=['rencana_tahun', 'rencana_subopd', 'rencana_kegiatan'], name='unique_rencana_pusisa')
        ]
        db_table = 'pu_rencanapusisa'
        
class BaseRencanapuposting(models.Model):
    VERIF = [
        (1, 'Rencana Induk'),
        (2, 'Rencana Perubahan'),
    ]
    ...
    def get_satuan_kegiatan(self):
        return self.posting_subkegiatan.dausgpusub_satuan if self.posting_subkegiatan else None
    
    def get_subopd(self):
        return self.posting_subopd.sub_nama if self.posting_subopd else None
    
    def get_subkegiatan(self):
        return self.posting_subkegiatan.dausgpusub_nama if self.posting_subkegiatan else None
    
    def get_kegiatan(self):
        return self.posting_subkegiatan.dausgpusub_keg.dausgpukeg_nama if self.posting_subkegiatan else None
    
    def get_total_realisasi_pk(self):
        from pu.models import Realisasipu, Realisasipusisa

        model = None
        if self.__class__.__name__.endswith('sisa'):
            model = Realisasipusisa
        else:
            model = Realisasipu
...
class Rencanapuposting(BaseRencanapuposting):
    posting_rencanaid = models.ForeignKey('pu.Rencanapu', verbose_name='Id Rencana', on_delete=models.CASCADE)
    class Meta(BaseRencanapuposting.Meta):
        db_table = 'pu_rencanapuposting'

class Rencanapupostingsisa(BaseRencanapuposting):
    posting_rencanaid = models.ForeignKey('pu.Rencanapusisa', verbose_name='Id Rencana', on_delete=models.CASCADE)
    class Meta(BaseRencanapuposting.Meta):
        db_table = 'pu_rencanapupostingsisa'
    

class BaseRealisasipu(models.Model):
    VERIF = [
        (0, 'Diinput Dinas'),
        (1, 'Disetujui APIP'),
    ]
...
class Realisasipu(BaseRealisasipu):
    ...
    class Meta(BaseRealisasipu.Meta):
...
class Realisasipusisa(BaseRealisasipu):
    realisasi_rencanaposting = models.ForeignKey(Rencanapupostingsisa, verbose_name='Kegiatan', on_delete=models.CASCADE)
    realisasi_rencana = models.ForeignKey(Rencanapusisa, verbose_name="Id Rencana", on_delete=models.CASCADE, editable=False)
    ...
    def get_rencana_pk(self):
        ...
        nilai_rencana = Rencanapupostingsisa.objects.filter(filters).aggregate(total_nilai=Sum('posting_pagu'))['total_nilai'] or Decimal(0)
        # print(f"nilai rencana : {nilai_rencana}, {self.realisasi_subkegiatan_id} dan {filters}")
        return nilai_rencana
    ...
    def get_rencanaoutput_pk(self):
        ...
        nilai_output = Rencanapupostingsisa.objects.filter(filters).aggregate(total_nilai=Sum('posting_output'))['total_nilai'] or Decimal(0)
        # print(f"nilai rencana : {nilai_rencana} dan {filters}")
        return nilai_output
