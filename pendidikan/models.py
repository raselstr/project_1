...
class BaseRealisasi(models.Model):
    VERIF = [
        (0, 'Diinput Dinas'),
        (1, 'Disetujui APIP'),
    ]
    realisasi_tahun = models.IntegerField(verbose_name="Tahun",default=datetime.now().year)
    realisasi_dana = models.ForeignKey('dana.Subkegiatan', verbose_name='Sumber Dana',on_delete=models.CASCADE)
    realisasi_tahap = models.ForeignKey('dana.TahapDana', verbose_name='Tahap Realisasi',on_delete=models.CASCADE)
...
class Realisasi(BaseRealisasi):
    ...
    class Meta(BaseRealisasi.Meta):
        db_table = 'pendidikan_realisasi'
    ...
    def get_persenpagu(self, tahun, opd, dana, posting):
        try:
            totalrealisasi = self.get_realisasi_total(tahun, opd, dana)
            persenpagu = Rencanaposting().get_total_rencana(tahun, opd, dana, posting)
...
class Realisasisisa(BaseRealisasi):
    realisasi_rencanaposting = models.ForeignKey(Rencanapostingsisa, verbose_name='Kegiatan', on_delete=models.CASCADE)
    realisasi_rencana = models.ForeignKey(Rencanasisa, verbose_name="Id Rencana", on_delete=models.CASCADE, editable=False)
    class Meta(BaseRealisasi.Meta):
    ...
    def get_persenpagu(self, tahun, opd, dana, posting):
        try:
            totalrealisasi = self.get_realisasi_total(tahun, opd, dana)
            persenpagu = Rencanapostingsisa().get_total_rencana(tahun, opd, dana, posting)
