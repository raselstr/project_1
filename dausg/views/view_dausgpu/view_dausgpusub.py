...
from ...models import DausgpuKeg,DausgpuSub
from ...forms.form_dausgpu import DausgpuSubForm

from django.http import HttpResponse
from ...resources import DausgpuSubResource
from tablib import Dataset

Form_data = DausgpuSubForm
Model_data = DausgpuSub
Model_induk = DausgpuKeg
lokasitemplate = 'dausgpu/dausgpusub/dausgpusub_list.html'
lokasiupdate = 'dausgpu/dausgpusub/dausgpusub_edit.html'
tag_url = 'list_dausgpusub'

resource = DausgpuSubResource
...
def export(request):
    ...
    response['Content-Disposition'] = 'attachment; filename="DAU SG Pekerjaan Umum Sub Kegiatan.xlsx"'
    return response
...
def list(request, number, sub):
    ...
    dausgpu_keg = get_object_or_404(Model_induk, id=sub)
    data = dausgpu_keg.dausgpusubs.all()
    # data = Model_data.objects.all()
    form = Form_data(request.POST or None, sub=sub)
    
    context = {
        "judul": "Daftar Sub Kegiatan DAU SG Pekerjaan Umum", 
        'dausgpu_keg': dausgpu_keg,
        "tombol" : "Tambah Sub Kegiatan DAUSG Pekerjaan Umum",
        "form": form, 
        "datas": data,
        "number":number,
        "sub":sub,
