...
from ...models import DausgpuKeg, DausgpuProg
from ...forms.form_dausgpu import DausgpuKegForm

from django.http import HttpResponse
from ...resources import DausgpuKegResource
from tablib import Dataset

Form_data = DausgpuKegForm
Model_data = DausgpuKeg
Model_induk = DausgpuProg
lokasitemplate = 'dausgpu/dausgpukeg/dausgpukeg_list.html'
lokasiupdate = 'dausgpu/dausgpukeg/dausgpukeg_edit.html'
tag_url = 'list_dausgpukeg'
lokasiload = 'load/load_subrinckeg.html'

resource = DausgpuKegResource
...
def export(request):
    ...
    response['Content-Disposition'] = 'attachment; filename="DAU SG Pekerjaan Umum Kegiatan.xlsx"'
    return response
...
def list(request, number):
    ...
    data = dankel_prog.dausgpukegs.prefetch_related('dausgpusubs').all()
    # data = Model_data.objects.all()
    form = Form_data(request.POST or None, number=number)
    
    context = {
        "judul": "Daftar Kegiatan DAU SG Pekerjaan Umum", 
        'dankel_prog': dankel_prog,
        "tombol" : "Tambah Kegiatan DAUSG Pekerjaan Umum",
        "form": form, 
        "datas": data,
        "number":number,
